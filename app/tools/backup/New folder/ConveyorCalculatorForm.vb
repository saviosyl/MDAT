...
Public Class ConveyorCalculatorForm
    ...
    ' ================= INIT =================
    Public Sub New()
        Me.Text = "MetaMech – Conveyor Calculator"
    ...
    Private Sub BuildUI()
        ...
        cmbEnv = AddCombo(y, {"Clean", "Dusty", "Wet", "Washdown"})
        y += 26

        txtDuty = AddInput("Duty Cycle (%)", y) : y += 26

        AddLabel("Operation Mode", y)
        cmbMode = AddCombo(y, {"Continuous", "Intermittent"})
        y += 26

        txtLoadVar = AddInput("Load Variability Factor", y) : y += 30

        AddSection("ADVANCED", y) : y += 26
        chkAdv = AddCheck("Enable Advanced Factors", y)
        AddHandler chkAdv.CheckedChanged, AddressOf ToggleAdvanced
        y += 24
        ...
        chkRel = AddCheck("Enable Reliability", y)
        AddHandler chkRel.CheckedChanged, AddressOf ToggleReliability
        y += 24

        txtReliability = AddInput("Reliability Factor", y) : y += 24
        txtLifetime = AddInput("Expected Lifetime (hrs)", y) : y += 36

        btnCalc = CreateButton("Calculate", PURPLE, New Point(20, y), AddressOf Calculate)
        btnPdf = CreateButton("Export PDF", GREEN, New Point(130, y), AddressOf ExportPdf)
        btnReset = CreateButton("Reset", YELLOW, New Point(240, y), AddressOf ResetAll)

        pnlInput.Controls.AddRange(New Control() {btnCalc, btnPdf, btnReset})

        pnlResults = New Panel With {
            .Location = New Point(390, 10),
            .Size = New Size(770, 660),
            .BackColor = BG
        }
        ...
        lblStatus = New Label With {
            .Text = "Status: Ready",
            .ForeColor = OK,
            .Location = New Point(390, 680),
            .AutoSize = True
        }
        Me.Controls.Add(lblStatus)

        ToggleAdvanced(Nothing, EventArgs.Empty)
        ToggleReliability(Nothing, EventArgs.Empty)
    ...
    Private Sub Calculate(sender As Object, e As EventArgs)
        ...
        If cmbEnv.Text = "Wet" Then envFactor = 1.3
        ...
        If chkAdv.Checked AndAlso Val(txtRoller.Text) > 0 Then
        ...
        If chkAdv.Checked AndAlso Val(txtEff.Text) > 0 Then eff = Val(txtEff.Text) / 100.0

        Dim sf As Double = 1.0
        If chkAdv.Checked AndAlso Val(txtSF.Text) > 0 Then sf = Val(txtSF.Text)

        Dim rel As Double = 1.0
        If chkRel.Checked AndAlso Val(txtReliability.Text) > 0 Then rel = Val(txtReliability.Text)
    ...
    Private Sub ExportPdf(sender As Object, e As EventArgs)
        Dim sfd As New SaveFileDialog With {.Filter = "PDF Files|*.pdf"}
    ...
    Private Sub ResetAll(sender As Object, e As EventArgs)
        For Each c As Control In pnlInput.Controls
            ...
            If TypeOf c Is ComboBox Then DirectCast(c, ComboBox).SelectedIndex = 0
    ...
    Private Function AddInput(t As String, y As Integer) As TextBox
        AddLabel(t, y)
        Dim tb As New TextBox With {.Location = New Point(170, y - 2), .Width = 160, .BackColor = BG, .ForeColor = TXT}
    ...
    Private Function AddCheck(t As String, y As Integer) As CheckBox
        Dim cb As New CheckBox With {.Text = t, .ForeColor = TXT, .Location = New Point(14, y)}
    ...
    Private Function AddCombo(y As Integer, items() As String) As ComboBox
    ...
    Private Function CreateButton(t As String, c As Color, p As Point, h As EventHandler) As Button
        Dim b As New Button With {.Text = t, .BackColor = c, .ForeColor = Color.White, .Location = p, .Size = New Size(90, 32)}
        AddHandler b.Click, h
        Return b
    End Function

    Private Sub ToggleAdvanced(sender As Object, e As EventArgs)
        Dim en As Boolean = chkAdv.Checked
        txtRoller.Enabled = en
        txtEff.Enabled = en
        txtSF.Enabled = en
    End Sub

    Private Sub ToggleReliability(sender As Object, e As EventArgs)
        Dim en As Boolean = chkRel.Checked
        txtReliability.Enabled = en
        txtLifetime.Enabled = en
    End Sub
